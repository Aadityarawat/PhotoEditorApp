...
import com.example.photoeditor.dashBoard.Adapter.CropToolType
import com.example.photoeditor.dashBoard.Adapter.TextViewAdapter
import com.example.photoeditor.others.ColorPickerAdapter
import com.example.photoeditor.R

class TextEditorDialogFragment : DialogFragment(), TextViewAdapter.OnTextItemListener {
    ...
    private val mTextViewAdapter = TextViewAdapter(this)
    private lateinit var rvCropView : RecyclerView
    private lateinit var addTextColorPickerRecyclerView: RecyclerView
    private var size = 11F
    private var count = 0
    private val array = arrayOf(R.font.nunito_bold,
        R.font.nunito_bold_italic,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        rvCropView =
            view.findViewById(R.id.rvCropView)
        val layoutManager1 = LinearLayoutManager(activity, LinearLayoutManager.HORIZONTAL, false)
        rvCropView.layoutManager = layoutManager1
        rvCropView.setHasFixedSize(true)
        rvCropView.adapter = mTextViewAdapter
    ...
    override fun onCropToolSelected(toolType: CropToolType) {
        when(toolType){
            CropToolType.FONT_SIZE -> {
                size += 5F
                mAddTextEditText.textSize = size
            }
            CropToolType.FONT_FAMILY -> {
                if (count > 4){
                    count = 0
                }
                val typeface = resources.getFont(array[count])
                mAddTextEditText.setTypeface(typeface)
                count++

            }
            CropToolType.BOLD -> {
                mAddTextEditText.setTypeface(mAddTextEditText.typeface, Typeface.BOLD)
            }
            CropToolType.ITALIC -> {
                mAddTextEditText.setTypeface(mAddTextEditText.typeface, Typeface.ITALIC)
            }
            CropToolType.BG_COLOUR -> {
                mAddTextEditText.background = ColorDrawable(R.color.blue)
            }
            CropToolType.TEXT_COLOR -> {
                addTextColorPickerRecyclerView.visibility = View.VISIBLE
                rvCropView.visibility = View.GONE
            }
        }
    }
}